...
We chose 4 VNH5050AE motor drivers to control our motors because it was a smaller package of our tried and tested VNH5019A Motors which are still serverly overpriced at the time of writing. The Australian team from Brisbane Boys' College Robotics [LJStand](https://ljstand.com/) also used the same motor drivers so we were relatively confident that these motors would work.

However, during our testing phase, we discovered that when paired with power-hungry Joinmax motors, these motor drivers were not well-suited and resulted in significant overheating issues. To address this problem, we had to add passive aluminum heat sinks to the motor drivers to prevent them from overheating.

The rest of our circuits are relatively simple and are not worth mentioning.
