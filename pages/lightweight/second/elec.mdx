...
The schmatics was still a copy from ours pervious year's robot becuase we did not have the time to learn both mechanical and electrical within a short timespan.
...
Instead of putting most of the components on the top Layer eg. STM32, MUX, we opted to place it on the bottom side of the PCB for easier wiring. Although some of our sneiors are against where by the STM32 might be too close to the ground and will sratch or tear it off, we realise that we gave enough clearance for it to not be a major issue.
